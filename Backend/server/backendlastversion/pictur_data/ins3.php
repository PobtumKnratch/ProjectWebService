...
if (isset($_REQUEST['delete_id'])) {
    ...
    $select_stm = $cont->prepare('SELECT * FROM fileupload3 WHERE id = :id');
    $select_stm->bindParam(':id', $id);
    $select_stm->execute();
    $row = $select_stm->fetch(PDO::FETCH_ASSOC);
    unlink("fileupload3/" . $row['image']);

    $delete_stm = $cont->prepare('DELETE FROM fileupload3 Where id = :id');
    $delete_stm->bindParam(':id', $id);
    $delete_stm->execute();

    header("Location: ins3.php");
}
...
<body>
    ...
    <div class="row">
        <div class="col">
            <a href="upsib.php" class="btn btn-success">Add image</a>
        </div>
        <div class="col-4">
    ...
    <table class="table table-striped table-bordered table-hover mt-3">
        ...
        <tbody>
            <?php
            $sql = "SELECT count(*) FROM fileupload3 ";
            try {
            ...
            // $sql = " SELECT * FROM fileupload3 where id ORDER BY id limit {$row_start},{$per_page}";
            // $select_stm = $cont->prepare($sql);
            // $select_stm->execute();

            $sql = " SELECT * FROM fileupload3 where name LIKE '%" . $strKeyword . "%' ORDER BY id limit {$row_start},{$per_page} ";
            $select_stm = $cont->prepare($sql);
            $select_stm->execute();
            ...
            ?>
                ...
                <tr style="font-family: 'Mali', cursive;">
                    <form action="show_data_signboard.php?update_id=<?php echo $row['id']; ?>" method="post" name="show">

                        <input type="hidden" name="id" value="<?php echo $row['id']; ?>" />
                        <input type="hidden" name="image" value="fileupload3/<?php echo $row['image']; ?>" />
                        ...
                        <td style=" vertical-align: middle;">
                            ...
                            <center><input type="image" name="type" value="1" src="fileupload3/<?php echo $row['image']; ?>" style="width: 100px; height: 80;" /></center>
                    ...
                    <form action="edit_signb.php?update_id=<?php echo $row['id']; ?>" method="post" name="update">
                        <input type="hidden" name="id" value="<?php echo $row['id']; ?>" />
                        <input type="hidden" name="image" value="fileupload3/<?php echo $row['image']; ?>" />
                        <input type="hidden" name="name" value="<?php echo $row['name']; ?>" />
                        <input type="hidden" name="tag" value="<?php echo $row['tag']; ?>" />
                        ...
                        <td style="width: 100px; vertical-align: middle;">
                            <center>
                                ...
                                <!-- <a href="JavaScript:{window.location='delete_pic3.php?id=<?php echo $row['id']; ?>';}" onclick="return confirm('คุณต้องการจะลบรูปภาพใช่หรือไม่? !!!')" class="btn btn-danger">Delete</a> -->
                            </center>
                        </td>
                ...
                <form action="delete_pic3.php?id=<?php echo $row['id']; ?>" method="post" name="delete">
