...
if (isset($_REQUEST['btn_insert'])) {
    try {
        ...
        $path = "fileupload4/" . $snewname . $img_file;
        ...
        $newname = "fileupload4/" . $snewname . "." . $ext;
        ...
        if (!isset($errorMsg)) {
            $stmt = $cont->prepare('INSERT INTO fileupload4 (name,image,tag) VALUE(:fname,:fimage,:tag) ');
            $stmt->bindParam(':fname', $name);
            $stmt->bindParam(':fimage', $newnametodb);
            $stmt->bindParam(':tag', $tag);
            if ($stmt->execute()) {
                $insertMsg = "File Uploaded Successfully...";
                header('refresh:0;scenario.php');
            }
        }
                        ...
                        <div class="form-group">
                            <center>
                                <div class="col-sm-12">
                                    ...
                                    <a href="scenario.php" class="btn btn-danger">Cancel</a>
                                </div>
                            </center>
                        </div>
